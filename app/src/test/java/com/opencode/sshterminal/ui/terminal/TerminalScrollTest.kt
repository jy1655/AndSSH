...
class TerminalScrollTest {
    ...
    @Test
    fun `swipe up increases scrollOffset toward history`() {
        val dragAmount = -90f
        ...
        assertTrue(
            "Swipe up (dragAmount<0) must increase scrollOffset, got ${result.newScrollOffset}",
            result.newScrollOffset > 0
        )
    }

    @Test
    fun `swipe down decreases scrollOffset toward live view`() {
        val dragAmount = 90f
        val result = computeScrollUpdate(
            dragAmount = dragAmount,
        ...
        assertTrue(
            "Swipe down (dragAmount>0) must decrease scrollOffset, got ${result.newScrollOffset}",
